...
import (
	...
	"github.com/fullcycle/chat/internal/domain/entity"
	"github.com/fullcycle/chat/internal/domain/gateway"
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	if err != nil {
		if err.Error() == "chat not found" {
			...
			return nil, errors.New("error feching existing chat" + err.Error())
		}
	}
	...
	for {
		...
		fullResponse.WriteString(reponse.Choices[0].Delta.Content)
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	initialMenssage, err := entity.NewMessage("system", input.Config.InitialSystemMessage, model)
	if err != nil {
		return nil, errors.New("error creating initial message:" + err.Error())
	}
	chat, err := entity.NewChat(input.UserID, initialMenssage, chatConfig)
	if err != nil {
		return nil, errors.New("error creating new chat:" + err.Error())
	}
	return chat, nil
